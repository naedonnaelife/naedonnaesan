...
const KakaoMap: React.FC<KakaoMapProps> = ({ areaName }) => {
  ...
  useEffect(() => {
    ...
    const clusterer = new kakao.maps.MarkerClusterer({
      ...
      calculator: [2, 3, 5, 10, 30, 50, 100, 300, 500, 1000], // 클러스터의 크기 구분 값, 각 사이값마다 설정된 text나 style이 적용된다
      texts: ['2+','3+', '5+', '10+', '30+', '50+', '100+', '300+', '500+', '1000+'],
      styles: [
        {
    ...
    axios.get('/buildings', { params : {dongname: '신사동'}})
    .then((response)=>{
      console.log(response.data)
      const markers = response.data.object.map((position: { x: number; y: number }) => {
